...
class Trajectory:
    def __init__(self):
        ...
        self.planner_uav_state_pub = rospy.Publisher(
    ...
    def get_desired(self, mode, states, x_offset, yaw_offset):
        ...
        self.publish_planner_uav_state()#0
    ...
    def publish_planner_uav_state(self):  #0
        """
        Publish estimated/controller UAV state to planner.
        trajectory/controller frame is NED-like:
        x: same as Gazebo world x
        y: opposite sign of Gazebo world y
        """
        current_b1 = self.get_current_b1()
        yaw_ctrl = np.arctan2(current_b1[1], current_b1[0])

        msg = Float32MultiArray()

        # controller/NED-like -> planner/Gazebo world
        x_world = float(self.x[0])
        y_world = float(-self.x[1])

        # yaw 先用這個版本；如果 LiDAR 前方左右反掉，再把負號拿掉測試
        yaw_world = float(-yaw_ctrl)
        ...
        self.planner_uav_state_pub.publish(msg)
