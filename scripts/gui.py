...
class Gui():
    def __init__(self):
        ...
        self.modes = ['Idle', 'Warm-up', 'Take-off', 'Land', 'Stay', 'Circle', 'Tag-Land', 'Search-and-Land', 'planner_path_follow', 'online_planner_mission']
        self.rdo_mode = []
    ...
    def on_key_press(self, widget, event):
        ...
        if key == Gdk.KEY_M or  key == Gdk.KEY_m:
            ...
            self.rdo_mode[7].set_active(True)
            rover.x_offset = np.zeros(3)
            rover.yaw_offset = 0.0
        elif key == Gdk.KEY_8:
            self.rdo_mode[8].set_active(True)
            rover.x_offset = np.zeros(3)
            rover.yaw_offset = 0.0
        elif key == Gdk.KEY_9:
            self.rdo_mode[9].set_active(True)
            rover.x_offset = np.zeros(3)
            rover.yaw_offset = 0.0
